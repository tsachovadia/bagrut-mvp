...
// Paths
const DATA_DIR = path.resolve(__dirname, '../../data/universities/haifa');
...
interface HaifaRules {
    universityKey: string;
    sekemAlgorithm: any;
    bagrutBonuses: any[];
}

async function seedHaifa() {
    console.log('⛰️ Starting Haifa Seed Script...');

    try {
        const structureRaw = fs.readFileSync(STRUCTURE_PATH, 'utf-8');
        const structure: HaifaStructure = JSON.parse(structureRaw);

        const rulesRaw = fs.readFileSync(RULES_PATH, 'utf-8');
        const rules: HaifaRules = JSON.parse(rulesRaw);
        ...
        if (existingRules) {
            ...
            console.log(`✅ Updated existing Admission Rules for Haifa`);
        } else {
            await supabase
                .from('admission_rules')
                .insert({
            ...
            console.log(`✅ Created new Admission Rules for Haifa`);
        }
        ...
        for (const faculty of structure.university.faculties) {
            ...
            const insertProgram = async (progName: string, url: string, desc: string) => {
                ...
                // Heuristics for Haifa
                if (progName.includes('משפטים')) {
                    degreeType = 'LL.B';
                    duration = 3.5;
                } else if (progName.includes('מדעי המחשב') || progName.includes('מערכות מידע')) {
                    degreeType = 'B.Sc'; // Often B.Sc even in Haifa
                    duration = 3;
                } else if (faculty.name.includes('מדעי הטבע')) {
                    degreeType = 'B.Sc';
                    duration = 3;
                } else if (progName.includes('סיעוד') || progName.includes('פיזיותרפיה') || progName.includes('ריפוי בעיסוק')) {
                    degreeType = 'B.A'; // Usually B.A or B.S.N depending, keeping generic B.A/B.Sc
                    if (progName.includes('סיעוד')) degreeType = 'B.S.N';
                    duration = 4; // Health usually 3.5-4
                }
        ...
        console.log('✅ Haifa Data Ingestion Complete.');
