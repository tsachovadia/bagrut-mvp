...
// Paths
const DATA_DIR = path.resolve(__dirname, '../../data/universities/reichman');
...
interface ReichmanRules {
    universityKey: string;
    sekemAlgorithm: any;
    bagrutBonuses: any[];
}

async function seedReichman() {
    console.log('📐 Starting Reichman Seed Script...');

    try {
        const structureRaw = fs.readFileSync(STRUCTURE_PATH, 'utf-8');
        const structure: ReichmanStructure = JSON.parse(structureRaw);

        const rulesRaw = fs.readFileSync(RULES_PATH, 'utf-8');
        const rules: ReichmanRules = JSON.parse(rulesRaw);
        ...
        if (existingInst) {
            ...
            const { data: newInst, error: instError } = await supabase
                ...
                .insert({
                    ...
                    type: 'university', // Technically a private university, but 'university' type fits schema
                    website_url: structure.university.website,
                    logo_url: '/logos/reichman.png'
                })
                .select()
                .single();
        ...
        if (existingRules) {
            ...
            console.log(`✅ Updated existing Admission Rules for Reichman`);
        } else {
            await supabase
                .from('admission_rules')
                .insert({
            ...
            console.log(`✅ Created new Admission Rules for Reichman`);
        }
        ...
        for (const faculty of structure.university.faculties) {
            ...
            const insertProgram = async (progName: string, url: string, desc: string) => {
                ...
                // Heuristics for Reichman
                if (progName.includes('מדעי המחשב') || progName.includes('יזמות ומדעי המחשב')) {
                    degreeType = 'B.Sc';
                }
                ...
                // Double majors involving BSc usually grant BSc, but sometimes BA. Keeping simple logic.
                if (progName.includes('וכלכלה') && progName.includes('מנהל עסקים')) {
                    degreeType = 'B.A';
                }
        ...
        console.log('✅ Reichman Data Ingestion Complete.');
