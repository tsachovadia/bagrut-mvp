...
// Paths
const DATA_DIR = path.resolve(__dirname, '../../data/universities/openu');
...
interface OpenURules {
    universityKey: string;
    sekemAlgorithm: any;
    bagrutBonuses: any[];
}

async function seedOpenU() {
    console.log('📐 Starting OpenU Seed Script...');

    try {
        const structureRaw = fs.readFileSync(STRUCTURE_PATH, 'utf-8');
        const structure: OpenUStructure = JSON.parse(structureRaw);

        const rulesRaw = fs.readFileSync(RULES_PATH, 'utf-8');
        const rules: OpenURules = JSON.parse(rulesRaw);
        ...
        if (existingRules) {
            ...
            console.log(`✅ Updated existing Admission Rules for OpenU`);
        } else {
            await supabase
                .from('admission_rules')
                .insert({
            ...
            console.log(`✅ Created new Admission Rules for OpenU`);
        }
        ...
        for (const faculty of structure.university.faculties) {
            ...
            const insertProgram = async (progName: string, url: string, desc: string) => {
                let degreeType = 'B.A';
                let duration = 3; // OpenU is flexible, but 3-4 is standard equivalent

                // Heuristics for OpenU
                if (progName.includes('הנדסת')) {
                    degreeType = 'B.Sc';
                    duration = 4;
                } else if (faculty.name.includes('מדעי הטבע') || faculty.name.includes('מחשב')) {
                    degreeType = 'B.Sc';
                    // Physics/Chemistry can also be B.Sc or B.A depending, usually B.Sc in Israel for main track
                }

                if (progName.includes('ניהול') || progName.includes('כלכלה') || progName.includes('חשבונאות')) {
                    degreeType = 'B.A';
                }
        ...
        console.log('✅ OpenU Data Ingestion Complete.');
