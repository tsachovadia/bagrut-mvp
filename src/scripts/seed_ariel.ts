...
// Paths
const DATA_DIR = path.resolve(__dirname, '../../data/universities/ariel');
...
interface ArielRules {
    universityKey: string;
    sekemAlgorithm: any;
    bagrutBonuses: any[];
}

async function seedAriel() {
    console.log('📐 Starting Ariel Seed Script...');

    try {
        const structureRaw = fs.readFileSync(STRUCTURE_PATH, 'utf-8');
        const structure: ArielStructure = JSON.parse(structureRaw);

        const rulesRaw = fs.readFileSync(RULES_PATH, 'utf-8');
        const rules: ArielRules = JSON.parse(rulesRaw);
        ...
        if (existingRules) {
            ...
            console.log(`✅ Updated existing Admission Rules for Ariel`);
        } else {
            await supabase
                .from('admission_rules')
                .insert({
            ...
            console.log(`✅ Created new Admission Rules for Ariel`);
        }
        ...
        for (const faculty of structure.university.faculties) {
            ...
            const insertProgram = async (progName: string, url: string, desc: string) => {
                ...
                // Heuristics for Ariel
                if (progName.includes('הנדס')) {
                    degreeType = 'B.Sc';
                    duration = 4;
                } else if (progName.includes('רפואה') && !progName.includes('קדם')) { // Ensure not pre-med unless intended
                    degreeType = 'M.D';
                    duration = 7;
                } else if (progName.includes('ארכיטקטורה')) {
                    degreeType = 'B.Arch';
                    duration = 5;
                } else if (faculty.name.includes('מדעי הטבע') || progName.includes('מדעי המחשב')) {
                    degreeType = 'B.Sc';
                    duration = 3;
                } else if (progName.includes('סיעוד') || progName.includes('פיזיותרפיה')) {
                    degreeType = 'B.PT'; // or B.S.N
        ...
        console.log('✅ Ariel Data Ingestion Complete.');
